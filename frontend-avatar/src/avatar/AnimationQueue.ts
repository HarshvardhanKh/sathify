...
/** How far behind live speech the queue is allowed to fall (Phase 13:
 * "the live system should not slowly fall behind the video") before a
 * newly-arriving sequence drops the old backlog instead of queuing after it. */
...
/** A fingerspell MotionItem becomes one step per letter (plus a trailing
 * pause) instead of one flat item, so the queue's play loop stays uniform
 * ("always just play animation_id for duration_ms, advance") while still
 * visibly stepping through letters one at a time. */
  ...
  const enqueueSequence = useCallback(
    (seq: MotionSequence) => {
      ...
      if (backlogMs > STALE_THRESHOLD_MS) {
        // Prefer current (live) speech over a growing historical backlog.
        queueRef.current = [];
      }
